...
interface AuthContextType {
  ...
  completeOnboarding: (role: 'candidate' | 'recruiter', fullName: string, companyName?: string | null) => Promise<any>;
...
const mapBackendUser = (backendUser: any): User | null => {
  ...
  if (backendUser.fullName) {
    ...
    fullName = backendUser.profile.fullName || backendUser.profile.companyName || backendUser.profile.contactName || '';
  } else if (backendUser.candidateProfile) {
    fullName = backendUser.candidateProfile.fullName || '';
  } else if (backendUser.recruiterProfile) {
    fullName = backendUser.recruiterProfile.companyName || backendUser.recruiterProfile.contactName || '';
  }
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      try {
        ...
        const response = await get<ApiResponse>('/auth/me', { _skipRedirect: true } as any);
        ...
        try {
          const res = await post<ApiResponse<{ accessToken: string; refreshToken: string }>>(
            '/auth/refresh-token',
  ...
  const login = async (email: string, password?: string) => {
    try {
      ...
      if (res.success && res.data) {
        const { accessToken: newAccessToken, refreshToken: newRefreshToken, user: backendUser } = res.data;
  ...
  const loginWithGoogle = async (supabaseAccessToken: string) => {
    try {
      const res = await post<ApiResponse<LoginResponse>>('/auth/google-login', { supabaseAccessToken });
      if (res.success && res.data) {
        const { accessToken: newAccessToken, refreshToken: newRefreshToken, user: backendUser } = res.data;
  ...
  const completeOnboarding = async (role: 'candidate' | 'recruiter', fullName: string, companyName?: string | null) => {
    try {
      const currentRefreshToken = localStorage.getItem('refreshToken') || refreshToken;
      if (!currentRefreshToken) {
        throw new Error('Không tìm thấy phiên làm việc tạm thời. Vui lòng đăng nhập lại.');
      }

      const res = await post<ApiResponse<LoginResponse>>('/auth/complete-onboarding', {
        role,
        fullName,
        companyName,
  ...
  const logout = async () => {
    try {
      ...
      if (storedRefreshToken) {
        await post<ApiResponse>('/auth/logout', { refreshToken: storedRefreshToken });
      }
    } catch (error) {
