...
const INITIAL_CANDIDATES: Candidate[] = [
  ...
  { 
    ...
    score: 4,
    internalNotes: 'Tư duy sản phẩm tốt, đã làm Fintech, khả năng giao tiếp ngoại ngữ xuất sắc.',
    feedbackMsg: '',
    feedbackDate: ''
  },
  { 
    ...
    score: 5,
    internalNotes: 'Kỹ thuật xuất sắc, giải quyết bài test nhanh, có kinh nghiệm triển khai CI/CD hệ thống lớn.',
    feedbackMsg: 'Chúc mừng bạn Lê Hoàng Hải đã chính thức trúng tuyển vào vị trí Senior Frontend Engineer. Ban Nhân sự sẽ liên hệ trao đổi chi tiết về Offer Letter.',
  ...
  { 
    ...
    score: 2,
    internalNotes: 'Kinh nghiệm tuyển dụng mảng Tech còn mỏng, chưa đáp ứng được nhu cầu mở rộng quy mô lớn hiện tại.',
    feedbackMsg: 'Cảm ơn bạn đã quan tâm đến cơ hội tại HireArch. Rất tiếc thời điểm hiện tại định hướng của bạn chưa phù hợp hoàn toàn với vị trí này.',
...
export function ManageCandidatesPage() {
  ...
  const handleSaveEvaluation = () => {
    ...
    showToast(`Đã lưu kết quả đánh giá cho ứng viên ${selectedCandidate.name} thành công!`);
  };
  ...
  const handleSendFeedback = () => {
    ...
    showToast(`Đã gửi phản hồi chính thức & cập nhật trạng thái ứng tuyển thành công!`);
  };
  ...
  const filteredCandidates = candidateList.filter(c => {
    ...
    const matchesPosition = positionFilter ? c.position === positionFilter : true;
    ...
    <div className="p-8">
      ...
      {toastMessage && (
        <div className="fixed top-24 right-8 bg-slate-900 text-white border-l-4 border-indigo-500 px-5 py-4 shadow-xl z-50 flex items-center gap-3 rounded-r-md animate-slide-left max-w-md">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Hệ thống thông báo</p>
            <p className="text-[12px] font-semibold text-slate-100 mt-0.5 leading-normal">{toastMessage}</p>
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-slate-500 mt-1">Theo dõi, chấm điểm đánh giá và phản hồi hồ sơ ứng tuyển.</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-0 bg-white border border-slate-200 border-b-0 px-5 py-4">
        ...
        <select
          ...
          className="h-9 px-3 border border-slate-200 text-[13px] outline-none text-slate-500 bg-white appearance-none cursor-pointer min-w-[170px] rounded-sm"
        >
      ...
      <div className="bg-white border border-slate-200 border-t-0 rounded-b-sm overflow-hidden">
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/75">
              <th className="w-10 px-5 py-3.5 text-center">
                <input type="checkbox" checked={allChecked} onChange={toggleAll} className="accent-indigo-600 cursor-pointer" />
              </th>
              {['Ứng viên', 'Vị trí ứng tuyển', 'Ngày ứng tuyển', 'Đánh giá nội bộ', 'Trạng thái', 'Thao tác'].map(h => (
          ...
          <tbody>
            {filteredCandidates.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-5 py-12 text-center text-slate-400 text-xs font-semibold">
                  Không tìm thấy ứng viên nào phù hợp với bộ lọc
                </td>
              </tr>
            ) : (
              filteredCandidates.map((c, i) => (
                <tr key={c.id} className={`border-b border-slate-100 transition-colors ${selected.includes(i) ? 'bg-indigo-50/30' : 'hover:bg-slate-50/60'}`}>
                  <td className="w-10 px-5 py-4 text-center">
                    <input type="checkbox" checked={selected.includes(i)} onChange={() => toggleRow(i)} className="accent-indigo-600 cursor-pointer" />
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 shadow-3xs" style={{ backgroundColor: c.color }}>
                        <span className="text-white text-[11px] font-bold">{c.initials}</span>
                      </div>
                      <div>
                        <p className="text-[13px] font-bold text-slate-800 leading-none">{c.name}</p>
                        <p className="text-[11px] text-slate-400 font-medium mt-1">{c.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-[13px] text-slate-700 font-semibold">{c.position}</td>
                  <td className="px-4 py-4 text-[13px] text-slate-500 font-medium">{c.date}</td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-0.5">
                      {c.score > 0 ? (
                        ...
                        <span className="text-[11px] text-slate-400 font-semibold italic">Chưa đánh giá</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-sm ${c.statusStyle}`}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: c.status === 'Mới' ? '#3b82f6' : c.status === 'Đang phỏng vấn' ? '#f97316' : c.status === 'Đạt' ? '#10b981' : '#ef4444' }} />
                      {c.status}
                    </span>
                  </td>
                  <td className="px-4 py-4">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-slate-50/50">
          <p className="text-[13px] text-slate-400">Hiển thị {filteredCandidates.length} ứng viên trong danh sách</p>
          <div className="flex items-center gap-1">
            <button className="h-8 px-3 text-[13px] text-slate-500 border border-slate-200 hover:bg-slate-50 transition-colors rounded-sm">Trước</button>
            <button className="w-8 h-8 text-[13px] border bg-indigo-600 text-white border-indigo-600 rounded-sm">1</button>
            <button className="h-8 px-3 text-[13px] text-slate-500 border border-slate-200 hover:bg-slate-50 transition-colors rounded-sm">Sau</button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-lg w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] border border-slate-100 dark:border-slate-800 animate-scale-up">
            ...
            <div className="w-full md:w-5/12 bg-slate-50 dark:bg-slate-950 p-6 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between overflow-y-auto">
              <div>
                <div className="flex items-center justify-between mb-6">
                  <span className="text-[10px] font-black text-slate-400 tracking-wider uppercase">Hồ sơ ứng viên</span>
                  <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-sm ${selectedCandidate.statusStyle}`}>
                    {selectedCandidate.status}
                  </span>
                </div>

                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white text-lg font-black shadow-md" style={{ backgroundColor: selectedCandidate.color }}>
                    {selectedCandidate.initials}
                  </div>
                  <div>
                    <h2 className="text-lg font-extrabold text-slate-900 dark:text-white leading-tight">{selectedCandidate.name}</h2>
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 font-bold mt-1 uppercase tracking-wider">{selectedCandidate.position}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 border-t border-slate-250 dark:border-slate-800 pt-5">
                  <div className="flex items-center gap-3 text-slate-600 dark:text-slate-350">
                    ...
                    <span className="text-xs font-semibold">{selectedCandidate.email}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600 dark:text-slate-350">
                    <Phone className="w-4 h-4 text-slate-400 shrink-0" />
                    <span className="text-xs font-semibold">{selectedCandidate.phone}</span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-600 dark:text-slate-350">
                    <Calendar className="w-4 h-4 text-slate-400 shrink-0" />
                    <span className="text-xs font-semibold">Nộp ngày: {selectedCandidate.date}</span>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-md mt-6 shadow-3xs">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-bold text-slate-800 dark:text-slate-100">CV_Nguyen_Van_A.pdf</p>
                      <p className="text-[10px] text-slate-400 font-medium">Bản chính thức • 2.4 MB</p>
                    </div>
                  </div>
            ...
            <div className="w-full md:w-7/12 p-6 flex flex-col justify-between overflow-y-auto">
              ...
              <div className="flex items-center justify-between border-b border-slate-150 pb-3 mb-5">
                <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 tracking-wide uppercase">Xử lý & Chấm Điểm Hồ Sơ</h3>
                <button 
              ...
              <div className="space-y-6 flex-1 pr-1">
                ...
                <div className="bg-indigo-50/20 dark:bg-slate-950/20 border border-indigo-100 dark:border-slate-800 p-5 rounded-md">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider">Chấm Điểm & Nhận Xét Nội Bộ</h4>
                  </div>
                  
                  {/* Star selection */}
                  <div className="mb-4">
                    <span className="text-[11px] font-extrabold text-slate-500 uppercase block mb-1.5">Chấm điểm sao cho ứng viên</span>
                    <div className="flex items-center gap-1.5">
                      ...
                      {tempScore > 0 && (
                        <span className="text-xs text-amber-500 font-bold ml-2">({tempScore} / 5 Sao)</span>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Notes content */}
                  <div>
                    <span className="text-[11px] font-extrabold text-slate-500 uppercase block mb-1.5">Ghi chú / Nhận xét nội bộ (Bộ phận Tuyển dụng)</span>
                    <textarea
                      value={tempNotes}
                ...
                <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-200 dark:border-slate-800 p-5 rounded-md">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider">Phản hồi chính thức cho Ứng viên</h4>
                  </div>

                  {/* Select status */}
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                      <span className="text-[11px] font-extrabold text-slate-500 uppercase block mb-1.5">Cập nhật trạng thái tuyển dụng</span>
                      <select
                        value={tempStatus}
                        onChange={e => {
                      ...
                      >
                        ...
                        <option value="Không phù hợp">Từ chối (Không phù hợp)</option>
                      </select>
                    </div>
                    ...
                    <div className="flex items-end">
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5 text-indigo-500" /> Tải mẫu thư tự động
                      </button>
                    </div>
                  </div>
                  ...
                  {/* Feedback Message */}
                  <div>
                    <span className="text-[11px] font-extrabold text-slate-500 uppercase block mb-1.5">Nội dung phản hồi (Sẽ gửi qua email/hệ thống cho ứng viên)</span>
                    <textarea
                  ...
                  <div className="flex justify-end mt-4">
                    ...
                    >
                      <Send className="w-3.5 h-3.5" /> Gửi phản hồi & Cập nhật trạng thái
                    </button>
                  </div>
                </div>
