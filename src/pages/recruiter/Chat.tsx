...
const INITIAL_CHANNELS: Channel[] = [
  {
    ...
    lastMessage: "Dạ em chào anh/chị, em đã gửi đầy đủ portfolio rồi ạ. Rất mong được phản hồi từ công ty.",
    time: "10:15",
    unread: 1,
    online: true,
    replies: [
  ...
  },
  {
    ...
    lastMessage: "Chào anh/chị, em muốn hỏi một chút về lộ trình phát triển sản phẩm của team mình được không ạ?",
    time: "Hôm qua",
    unread: 0,
    online: false,
    replies: [
  ...
  },
  {
    ...
    lastMessage: "Dạ em đã nhận được thư mời phỏng vấn kỹ thuật từ HR, hẹn gặp anh chị vào chiều mai.",
    time: "08/05",
    unread: 0,
    online: true,
    replies: [
  ...
  },
  {
    ...
    lastMessage: "Em cảm ơn công ty đã tạo cơ hội, em gửi lại mẫu khảo sát văn hóa doanh nghiệp ở đây ạ.",
    time: "05/05",
    unread: 0,
    online: false,
    replies: [
...
export function RecruiterChatPage() {
  ...
  const [messages, setMessages] = useState<{ [key: number]: Message[] }>({
    1: [
      { id: 1, sender: "them", text: "Xin chào quý công ty, em là Nguyễn Văn Thái, vừa nộp hồ sơ vào vị trí Senior Frontend Engineer ạ.", time: "09:30" },
      { id: 2, sender: "me", text: "Chào Thái, anh đã nhận được hồ sơ của em từ hệ thống HireArch. CV của em rất ấn tượng với nhiều kinh nghiệm React và Tailwind.", time: "09:45" },
      { id: 3, sender: "them", text: "Dạ em chào anh/chị, em đã gửi đầy đủ portfolio rồi ạ. Rất mong được phản hồi từ công ty.", time: "10:15" },
    ],
    2: [
      { id: 1, sender: "them", text: "Em chào HR ạ, em vừa ứng tuyển vị trí Product Manager. Em có gửi kèm link Case Study em đã làm.", time: "15:30" },
      { id: 2, sender: "me", text: "Cảm ơn Mai đã gửi thông tin. HR đã chuyển bài Case Study của em cho ban sản phẩm đánh giá nhé.", time: "15:45" },
      { id: 3, sender: "them", text: "Chào anh/chị, em muốn hỏi một chút về lộ trình phát triển sản phẩm của team mình được không ạ?", time: "Hôm qua" },
    ],
    3: [
      { id: 1, sender: "me", text: "Chào Hải, kết quả bài test thuật toán của em đạt 90/100 điểm. Anh muốn lên lịch phỏng vấn Technical Interview 1-1.", time: "03/05" },
      { id: 2, sender: "them", text: "Dạ tuyệt vời quá ạ! Em có thể sắp xếp phỏng vấn vào các buổi chiều từ 14:00 trở đi.", time: "04/05" },
      { id: 3, sender: "me", text: "Ok em, anh đặt lịch phỏng vấn lúc 15:00 ngày mai nhé. Link Zoom sẽ được hệ thống gửi tự động.", time: "05/05" },
      { id: 4, sender: "them", text: "Dạ em đã nhận được thư mời phỏng vấn kỹ thuật từ HR, hẹn gặp anh chị vào chiều mai.", time: "08/05" },
    ],
    4: [
      { id: 1, sender: "me", text: "Chào Phương Anh, em vui lòng hoàn thành bản khảo sát hành vi văn hóa trước khi phỏng vấn vòng 2 nhé.", time: "04/05" },
      { id: 2, sender: "them", text: "Em cảm ơn công ty đã tạo cơ hội, em gửi lại mẫu khảo sát văn hóa doanh nghiệp ở đây ạ.", time: "05/05" },
    ]
  });
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const responseText = autoReplies[Math.floor(Math.random() * autoReplies.length)] || "Dạ vâng ạ, em đã ghi nhận thông tin và sẽ chuẩn bị kỹ càng.";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold truncate uppercase mt-0.5 leading-none">{chan.position}</p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate mt-1.5 leading-snug">{chan.lastMessage}</p>
                    </div>
          ...
          <div className="h-16 px-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0 shadow-3xs">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold mt-1 leading-none uppercase tracking-wider text-xs text-indigo-600 dark:text-indigo-400">
            ...
            <div className="flex items-center gap-2">
              <button className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-not-allowed" title="Tính năng phỏng vấn thoại">
                <Phone className="w-4 h-4" />
              </button>
              <button className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-not-allowed" title="Tính năng phỏng vấn Video">
                <Video className="w-4 h-4" />
              </button>
              <button className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer" title="Thông tin ứng tuyển">
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"} animate-fade-in`}>
                  <div className={`flex items-end gap-2 max-w-[70%] ${isMe ? "flex-row-reverse" : ""}`}>
                    {/* Bong bóng tin nhắn */}
                    <div className={`rounded-2xl px-4 py-2.5 text-xs font-medium shadow-3xs leading-relaxed ${isMe
            ...
            {isTyping && (
              <div className="flex justify-start animate-fade-in">
                <div className="flex items-center gap-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-2.5 shadow-3xs">
                  <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500">{activeChannel.name} đang nhập tin nhắn</span>
