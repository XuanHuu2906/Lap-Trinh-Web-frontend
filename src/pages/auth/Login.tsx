...
export function LoginPage() {
  ...
  const [errorMessage, setErrorMessage] = useState("Email hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại.");
  ...
  const handleLoginSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      navigate("/candidate/overview", { replace: true });
    } catch (error) {
        ...
        <div className="w-full max-w-100 bg-white border border-slate-200/80 rounded-lg shadow-sm p-6 relative">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-11 bg-slate-900 hover:bg-slate-800 disabled:opacity-60 text-white font-bold text-[15px] rounded-full transition-all flex items-center justify-center gap-2 cursor-pointer shadow-3xs mt-2 active:scale-[0.98]"
            >
