      ...
      {isLoading ? (
        ...
        <div className="flex min-h-72 flex-col items-center justify-center border border-slate-800 bg-slate-900 px-4 text-center">
          ...
          <p className="mt-2 max-w-md text-sm text-slate-500">
            Nếu bảng CVTemplate đang trống, frontend sẽ không hiển thị dữ liệu
            giả nữa. Hãy thêm template ở backend/admin để candidate dùng.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredTemplates.map((template) => (
            <article
              key={template.id}
              className="border border-slate-800 bg-slate-900 transition hover:border-blue-500/70"
            >
            ...
            <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {previewTemplate.description || "Mẫu CV chưa có mô tả."}
                </p>
              </div>
